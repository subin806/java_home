...
public class ServerEx {
	public static void main(String[] args) {
		...
		try {
			//파일 입출력, 네트워크 통신을 할 때
			//try 구문으로 감싸야 합니다
			//서버 입장에서 포트번호 : 9999 라는 번호로
			//문을 열어 두기
			listener = new ServerSocket(9999); // 서버 소켓 생성
			System.out.println("연결을 기다리고 있습니다.....");
			// 클라이언트로부터 연결 요청 대기
			//상대방으로부터 소켓 연결이 온다면 이 메서드가 동작하고 서로 간에 소켓으로 연결됨
			//스트림, 연결관이 생겼다
			socket = listener.accept();
			System.out.println("연결되었습니다.");
			 //socket.getInputStrea() 상대방으로부터 전달 받은 데이터를 읽는 도구-> 메모리 임시 저장
			//new InputStreamReader -> 바이트로 읽은 코드를 다시 읽어서 문자 기반으로 읽기
		    //BufferedReader -> 보조 스트림등을 이용해서, 문자 기반으로 읽기
			//상대방으로부터 전달 받은 데이터를 갖고 있다. -> 메로리 상에
